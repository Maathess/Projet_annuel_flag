...
def import_dataset():
    dataset_folder = "F:/2021_3A_IABD2_FakeDataset"
    ...
    import_images_and_assign_labels(
        os.path.join(train_folder, "burger"), 1.0, X_train, y_train
    )
    import_images_and_assign_labels(
        os.path.join(train_folder, "sandwich"), -1.0, X_train, y_train
    )
    X_test = []
    y_test = []
    import_images_and_assign_labels(
        os.path.join(test_folder, "burger"), 1.0, X_test, y_test
    )
    import_images_and_assign_labels(
        os.path.join(test_folder, "sandwich"), -1.0, X_test, y_test
    )
